    ...
    <Form :validation-schema="userform">
      <div class="form-group">
        ...
        <Field type="text" class="form-control border border-primary" style="border-radius: 25px; height: 50px"
          id="nameproduct" name="name" v-model="infouser.username" placeholder="Enter name product" />
        <ErrorMessage name="name" class="text-danger" />
      </div>
      <div class="form-group">
        <label for="nameproduct">Email:</label>
        <Field type="text" class="form-control border border-primary" style="border-radius: 25px; height: 50px"
          id="nameproduct" name="email" v-model="infouser.email" placeholder="Enter name product" />
        <ErrorMessage name="email" class="text-danger" />
      </div>
      <button type="submit" class="btn btn-primary mt-2" @click="updateUser">Cập Nhật</button>
      <br>
    </Form>
  </div>
</template>